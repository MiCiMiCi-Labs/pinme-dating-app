...
- Mobile：Expo SDK 54、React Native 0.81.5、Expo Router、Supabase client、LiveKit React Native，iOS bundle identifier `com.micimici.pinme`。
...
- Headers：`apns-push-type: voip`、`apns-topic: com.micimici.pinme.voip`、`apns-priority: 10`、`apns-expiration: 当前时间 + 45 秒`
...
2. 找到或创建 App ID：`com.micimici.pinme`。
...
| 后端：APNs VoIP Push 发送 | `apps/backend/src/lib/apnsVoip.ts`：仅用 `node:http2`/`node:crypto`（无第三方 APNs/JWT 依赖）。**配置**：`APNS_TEAM_ID`/`APNS_KEY_ID`/`APNS_PRIVATE_KEY_BASE64`/`APNS_BUNDLE_ID`/`APNS_VOIP_TOPIC` 延迟读取；缺失/Base64 或 PEM 无效/私钥非 EC-P256（`asymmetricKeyType==='ec'` 且 `namedCurve==='prime256v1'`）/`voipTopic !== bundleId+'.voip'` 时返回分类化 `skipped`/`ok:false` 原因，不发起网络请求、不抛异常，仅记录一条只含分类 reason（如 `missing_config`/`topic_mismatch`）的 `console.warn`，不输出配置值/密钥。**JWT**：ES256、IEEE-P1363 raw 64-byte 签名（非 DER），50 分钟阈值缓存并按 team/key/private key 指纹失效。**发送**：按每条 `VoipDeviceToken.environment` 选择 sandbox/production endpoint；`sendVoipPushToDeviceToken` 用 `finishGraceful`（正常收到完整 response：clear timer + `session.close()`）与 `finishForced`（timeout/session error/stream error/同步异常：clear timer + `req.close(NGHTTP2_CANCEL)` + `session.destroy()`，均吞掉自身 cleanup 异常）两条幂等收尾路径，覆盖 session 在 `request()` 之前就报错的情况；payload build/JWT 签发/`session.request`/`req.end` 的同步异常全部捕获为结构化 `failed`，函数真正做到不 reject/不 throw。**Payload**（当前实际实现，字段唯一、无历史遗留）：`{ aps:{}, uuid, callId, callerName, handle, hasVideo }`——`uuid`/`callId` 同为 `Call.id`（CallKit UUID = Call.id），`handle`/`callerName` 为同一个经 Unicode code point 安全截断（64 字符）的展示名，均取自数据库、不含 token/用户 ID/照片等其他资料；按 `Buffer.byteLength(...,'utf8')` 校验最终序列化结果不超过 5120 字节（`MAX_VOIP_PAYLOAD_BYTES`），超限返回 `payload_too_large` 且不发起任何网络连接。**无效 token 清理**：HTTP 410 或 reason 为 `Unregistered`/`BadDeviceToken`/`DeviceTokenNotForTopic` 时，按 token+userId+platform+environment 精确删除；其他 4xx/5xx/超时不清理。**多设备**：`sendVoipPushForIncomingCall` 用 `Promise.allSettled` 并发发送，单 token 失败不影响其它；token 查询本身失败也返回结构化失败汇总，不 reject。**startCall 接入**：Call 创建并已提交、HTTP 201 响应发出之后再 `await` 调用（try/catch 包裹，永不影响已创建的 Call 或响应状态码）；若 `summary.failed>0`，记录一条只含 `{callId,total,sent,invalid,failed,skipped}` 的结构化 `console.warn`，不含 device token/JWT/private key/完整 payload/callerName。已用 **40 个纯单元测试**验证（`tests/apnsVoip.test.ts`，mock `node:http2`/`prisma`，不连接真实 APNs、不用真实 Apple secret）：config 校验各分支（含 RSA/错误 EC 曲线）、JWT header/claims/签名格式/缓存复用/阈值刷新/配置变更失效、endpoint 分流、headers/expiration/payload 精确字段、200/410/`DeviceTokenNotForTopic`/其他 4xx-5xx/stream error/session error/timeout 各分支、graceful/forced 两种清理路径与重复事件幂等、各同步异常、超限/emoji payload、多 token 混合结果与失效 token 精确清理、清理失败不影响汇总、APNs 未配置时的安全日志与不查 DB 不发网络请求。**真实 APNs 投递未做验证** | CODE COMPLETE |
...
| 移动端：扬声器切换 | `ConnectedCallScreen` 原第三个空白控制位替换为扬声器按钮，使用 `@livekit/react-native` 的 `AudioSession.getAudioOutputs()`/`selectAudioOutput()`（已读取当前安装版本类型声明确认 API，未引入新依赖）；切换前用 `getAudioOutputs()` 校验目标 deviceId 是否可用，不可用/调用失败时保留原状态并 toast 提示，不结束通话；**iOS 平台限制**：类型声明中 iOS 只暴露 `"default"`/`"force_speaker"` 两种 deviceId，没有真正的 "earpiece" 选项（OS 限制，非本实现缺陷），"speaker 关闭" 在 iOS 上等价于交还给系统默认路由（`default`），不是强制走听筒；Android 有对应的真实 `"speaker"`/`"earpiece"` deviceId。**未在真机/模拟器上验证实际音频路由效果**，仅完成类型检查与代码走查 | CODE COMPLETE（iOS 无真正 earpiece 选项，见说明；未做设备音频验证） |
| 移动端：CallKit 音频会话归属 + App Accept 同步 | 新增纯模块 `lib/callAudioCoordinator.ts`（`callAudioCoordinator.test.js`，node:assert）。**音频会话归属**：核对已安装 `@livekit/react-native` 真实源码确认 `registerGlobals()` 默认（`autoConfigureAudioSession: true`）安装的引擎级自动管理（`AudioManager.ts` `setupIOSAudioManagement`）才是 `AudioSession.start/stopAudioSession()` 的唯一实际调用者——绑定在 WebRTC 引擎自身 recording/playout 启停事件上，与挂载/卸载无关；`ConnectedCallScreen` 原有的手动 `AudioSession.startAudioSession()/stopAudioSession()` 挂载/卸载 effect 已删除（不是第二个 owner，避免双重调用）；核对已安装 `@livekit/components-react` 真实源码（`room-Q0GUQkyk.mjs`）确认 `<LiveKitRoom connect>` 的 `connect` prop 双向可靠（`false→true` 会重新 `room.connect()`，`true→false` 会 `room.disconnect()`），而 `audio` prop 在 `SignalConnected` 首次触发后再变化不会补发布/取消发布麦克风——因此选择只让 `connect` 随权限门控切换、`audio` 全程保持常量 `true`。**CallKit 权威信号**：核对已安装 `react-native-callkeep` 原生源码（`RNCallKeep.m` `didActivateAudioSession`/`didDeactivateAudioSession`）确认二者 payload 均不带 callId，且 `didActivateAudioSession` 会先发一条合成的 `AVAudioSessionInterruptionNotification`(Ended/ShouldResume) 再回调 JS——这正是 WebRTC 原生音频引擎据以恢复的信号，佐证"必须等 CallKit 授权才可连接"而非仅仅"少调用几次 start()"。`contexts/call.tsx` 新增 `ActiveCall.isCallKitManaged`、`callKitAudioState`（managedCallId/generation/audioActive）与 `handleNativeAudioSessionActivated/Deactivated`（由 coordinator 直接转发，不经队列，因为无 callId 也无需网络重试）；`isPrivateCallAudioAuthorized` 计算值传给 `voice-call-modal.tsx` 的 `<LiveKitRoom connect={...}>`；未经 CallKit 展示的普通前台通话恒为已授权（Phase 2 行为不变）；CallKit 展示的通话仅在 `didActivateAudioSession` 后授权，`didDeactivateAudioSession` 后收回授权（触发 `room.disconnect()`，`LiveKitConnectionSection` 的 `onDisconnected` 在未授权时跳过 `confirmCallOrFail`，避免把因门控主动断开误判为连接失败）；旧通话结束/身份切换/组件卸载经 `resolveCallKitManagedCallEnd` 清空追踪，`generation` 单调递增防止 A→B→A 串号，新通话固定从 `audioActive:false` 起步，绝不继承上一通话的授权。**App Accept 同步**：`contexts/call.tsx` 新增 `acceptCall` 包装（供纯 UI Accept 按钮使用，`handleNativeAnswer` 仍直接调用未包装的 `acceptCallAction`，这一非对称本身就是防止 CallKit 原生 answer 回声的机制，未额外加抑制标记）；服务端确认 `ACCEPTED` 且 `isCallKitManaged` 且未同步过时，`lib/voipPushKit.ts` 新增 `answerIncomingCallKitCall`（`CallKeep.answerIncomingCall`，同步 API，try/catch 包裹永不 throw）以 fire-and-forget 方式调用一次，用 `answerIncomingCallSyncedForRef` 保证每个 callId 最多一次，失败不影响已确认的服务端状态。**未在真机上验证**：CallKit 音频会话真实授权/收回时机、`didActivateAudioSession`/`didDeactivateAudioSession` 与 App 内 LiveKit 连接的真实相对时序、`answerIncomingCall` 对系统 CallKit UI 的真实观感 | CODE COMPLETE（未做真机 CallKit 音频/Accept 同步验证） |
| iOS：PushKit 集成 | token 注册/上传/失效、原生早期事件处理 | NOT STARTED |
| iOS：CallKit 集成 | `reportNewIncomingCall` 等全部生命周期回调；见下一行的编译状态 | IN PROGRESS |
| iOS：Expo config plugin | `apps/mobile/plugins/withVoipPushKit.js`：`UIBackgroundModes`(voip/remote-notification)、`@config-plugins/react-native-callkeep`、AppDelegate PKPushRegistryDelegate 注入。**第一次真实 EAS iOS Simulator development build**(build id `526021b8-1e51-4a42-935e-731cac2f1ff9`)已证明"Swift AppDelegate 直接引用 `RNCallKeep`/`RNVoipPushNotificationManager`"的方案编译失败(`cannot find 'RNCallKeep' in scope`、`cannot find 'RNVoipPushNotificationManager' in scope`)。**当前方案**改为最小 Objective-C bridge/shim(`PinMeVoipBridge.h`/`.m`，由插件在 prebuild 时生成并接入 Xcode Compile Sources + bridging header)：`.m` 直接 `#import` 两个第三方 pod 的真实 header 并调用其已核对存在的 selector(ObjC-to-ObjC，无 Swift 可见性问题)；`.h` 只暴露 Foundation/PushKit 类型的纯 C 函数，Swift 只调用这些 C 函数，不再出现 `RNCallKeep`/`RNVoipPushNotificationManager` 字样。`didInvalidatePushTokenFor` 保持显式空实现(两个库都没有对应的失效转发方法)。已用扩充后的纯 Node transformation 测试验证(`node plugins/withVoipPushKit.test.js`)：幂等性、Swift 输出不含两个第三方类名、只调用 bridge 函数、bridge 实现使用真实 selector、bridging header import 不重复、插件不触碰 Podfile(无 `withPodfile` 用法)。**这个 bridge 方案尚未经过第二次真实 EAS build 验证**——不得标记为 NATIVE BUILD VERIFIED；`#import <RNCallKeep/RNCallKeep.h>`/`#import <RNVoipPushNotification/RNVoipPushNotificationManager.h>` 在 `.m` 里是否能在无额外 HEADER_SEARCH_PATHS 情况下解析、bridging header 合并逻辑在真实已有 bridging header 的项目上是否正确，均未验证。本机 Windows 环境下 `expo prebuild --platform ios` 会直接报错跳过 iOS 原生工程生成("Skipping generating the iOS native project files. Run npx expo prebuild again from macOS or Linux")，因此插件的 pbxproj/bridging header 写入逻辑本身也未经过一次真实生成产物核实，只做了源码层面的 API 核对（`xcode`/`@expo/config-plugins` 包源码）。**第二次真实 EAS iOS Simulator development build**（build id `13a8fc41-761c-494d-b2b6-f2492f1edca6`）**失败，但与第一次失败的 Swift 可见性问题无关**——失败发生在 PREBUILD 阶段（`npm install`/`expo doctor` 均已成功，甚至还没到 pod install/Xcode 编译）：`TypeError: [ios.xcodeproj]: withIosXcodeprojBaseMod: Cannot read properties of null (reading 'path')`，堆栈精确定位到 `withVoipPushKit.js` 里 `project.addSourceFile(relativeImplPath, {target: nativeTarget.uuid})` 这一调用——省略了 `group` 参数导致内部走 `addPluginFile` 分支，而该分支要求项目里存在名为 `"Plugins"` 的 PBXGroup（`project.pbxGroupByName('Plugins').path`），但 Expo SDK 54 生成的工程模板里根本没有这个 group，导致 `.path` 读取 `null` 抛错。**这是插件自身对 `xcode` 包 API 的误用（config plugin/prebuild 阶段的 bug），不是 Swift/ObjC/bridging header 的问题**；Swift 能否看到 bridge 的 C 函数、`.m` 里两个第三方 header 能否被找到，这次构建完全没有机会验证。**已修复（尚未经过下一次 build 验证）**：改用 Expo 官方 `IOSConfig.XcodeUtils.addBuildSourceFileToGroup({filepath, groupName: projectName, project, targetUuid: nativeTarget.uuid, verbose: true})`（对照已安装的 `node_modules/@expo/config-plugins/build/ios/utils/Xcodeproj.d.ts`/`.js` 核实参数与行为），按 `projectName`（即 AppDelegate.swift 所在、必然存在的 app 自身 PBXGroup）而不是不存在的 `"Plugins"` group 添加文件，返回值重新赋回 `config.modResults`；helper 内部已有按 group 内同名文件去重的幂等逻辑。已扩充 `tests/withVoipPushKit.test.js` 做源码层面断言（不使用/不需要伪造 pbxproj fixture）：不再包含裸 `addSourceFile` 调用、确有 `addBuildSourceFileToGroup` 调用、`groupName: projectName`、`targetUuid: nativeTarget.uuid`、返回值确实赋回 `config.modResults`；原有 completion/UUID/bridge selector/幂等测试全部继续通过。**第三次真实 EAS iOS Simulator development build（build id `cb036e3a-40db-45e1-8d1d-6ef9e45c8a88`）成功**：`BUILD SUCCEEDED`。已下载并解码完整 Xcode 构建日志逐项核实（非仅看整体状态）：PREBUILD 未再崩溃；`PinMeVoipBridge.m` 的 `CompileC`（x86_64 与 arm64 两种架构）均出现在目标 `PinMe` 下且无 `error:` 诊断；`AppDelegate.swift` 的 `SwiftCompile` 正常执行；最终 `Ld .../PinMe`（x86_64 与 arm64）链接命令的库搜索路径中包含 `RNCallKeep`、`RNVoipPushNotification` 两个 pod 产物目录；全量日志里 `error:` 诊断计数为 0，且未出现任何 `RNCallKeep`/`RNVoipPushNotification`/`PinMeVoipBridge` 相关的 "not found"。**NATIVE BUILD VERIFIED（EAS iOS Simulator）。**

**明确仍未验证**：iPhone 真机构建与签名（本次全程是 simulator profile，未涉及任何 Apple 证书/provisioning）；PushKit token 注册；`/calls/devices` 上传；真实 APNs 投递；CallKit 在真机上的实际 UI 展示；App 处于后台/锁屏/冷启动时收到来电的完整链路。这些均需要后续单独的任务和两台真实 iPhone 才能验证，不能因为这次 simulator 编译成功而视为已完成。 | NATIVE BUILD VERIFIED（EAS iOS Simulator） |
